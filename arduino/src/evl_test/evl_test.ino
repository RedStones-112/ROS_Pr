...
void call_evl_LED() {
  ...
  for (int i = 0; i <= 3; i++) { // YELLOW_LED
    if (1.01 + (0.5 * i) < evl_loc && evl_loc < 1.49 + (0.5 * i)) { 
...
int min_floor_status() { // 가장 빠르게 누른층반환
  int result = 3;
  int min_floor = 0;
  for (int i = 0; i <= 2; i++) {
...
void move_evl() {
  ...
  if (target != 0.0 && count_time >= 500) {
    if (evl_loc - target > 0.0) { /// 엘리베이터 운행 방향
      move_status = -1;
      stop_count = 0;
    }
    else if (evl_loc - target < 0.0) {
      move_status = 1;
      stop_count = 0;
    }
    else {
  ...
  evl_loc = evl_loc + (0.1 * move_status);


  for (int i = 0; i <= 2; i++) {
    if (floor_status[i] != 0 && abs(evl_loc - 1.0 - static_cast<float>(i)) < 0.01) {
      
      pushed_buttons--;
      end_cancel_button(floor_status[i]);
      count_time = 0;
      move_status = 0;
      target = 0.0;
    }
