...
class WindowClass(QMainWindow, from_class) :
    def __init__(self):
        ...
        self.model = tf.keras.models.load_model("./SCT_model.h5")
    ...
    def clickCapture(self):
        if self.display_status == "Video":
            ...
            self.classisfication(self.image)
            
    def resize_and_preprocess_vgg(self, image):
        image = cv2.resize(image, dsize=(224, 224))
        image = img_to_array(image)
        image = image.reshape((1, image.shape[0], image.shape[1], image.shape[2]))
        return preprocess_input(image)
    
    def classisfication(self, image):
        image = self.resize_and_preprocess_vgg(image)
        yhat = self.model.predict(image)[0]
